...
search backend, use dtSearch request language \cite{doc:dtsearch} to specify the query. DtSearch options dtsSearchAnyWords or
...
If native full-text indexes are used, the following constructs can be used as
parts of query:
\begin{itemize}

\item phrases: several words in single or double quotes will be searched as a phrase
...
based on prefix tree (or \emph{trie}) conception. Main distinguishing features are:
\begin{enumerate}
...
\begin{note} BST  is very experimental at this moment; \textbf{do not use it
in production} or any critical applications now because BST has known bugs leading
to index corruption!
\end{note}
...
Sedna allows to build full-text indexes in order to combine XQuery with full-text search facilities. Resulting indexes need to be used explicitly via full-text search functions (see \ref{sec:ft-fun}), XQuery full text extensions are not supported.

Sedna can be integrated with dtSearch \cite{link:dtsearch-engine}, a commercial text
retrieval engine, which provides full-text indices. As dtSearch is a third party
commercial product, Sedna does not include dtSearch. If you are interested in
using Sedna with dtSearch, please contact us.

Below is the description of statements to manage full-text indices in
Sedna.
...
\verb!options! is a sting of the following form: \verb!"option=value{,option=value}"!. It denotes options used for index constuction. The following options are available:
\begin{itemize}
\item \verb!backend! -- specifies which implementation of full-text indexes to use. Allowed values are \verb!native! and \verb!dtsearch!, the latter in only available in dtSearch-enabled builds. For dtSearch-enabled builds, default backend is \verb!dtsearch!, for other builds - \verb!native!.
\end{itemize}
Options for \verb!native! backend:
\begin{itemize}
\item \verb!stemming! -- specifies stemming language to use.
\item \verb!stemtype! -- in order to be able to search both stemmed and original words - add \verb!stemtype=both! option, otherwise stemming will be used always if enabled.
\end{itemize}
