...
\begin{itemize}
\item Nodes kind: Element, Document, Text, Namespace and Attribute Nodes.
\item Atomic types:  xs:string , xs:boolean , xs:decimal (\emph{restricted}), xs:float, xs:double, xs:QName, xs:dateTime, xs:date, xs:time, xs:gYearMonth, xs:gYear, xs:gMonthDay, xs:gMonth, xs:gDay, xs:duration, xdt:yearMonthDuration, xdt:dayTimeDuration, xdt:untypedAtomic and xdt:anyAtomicType.
\end{itemize}
...
\item FLWOR: for and let clause, where clause, order by (there is a restriction that Let clause is prohibited in FLWOR-expressions which contain Order By clause), return clause.
\item Conditional Expression
\item Quantified Expression (without type declaration).
\item Expressions on SequenceTypes: Instance Of, Typeswitch, Cast, Castable, Treat
...
Sedna is integrated with dtSearch Engine \cite{link:dtsearch-engine}, a third party commercial product, to support full-text indices. As dtSearch works on Windows and Linux platforms, Sedna supports full-text indices only on these platforms. Below is the description of statements to manage full-text indices.
...
\bibitem{paper:data-model} ``XQuery 1.0 and XPath 2.0 Data Model'', W3C Candidate Recommendation, 3 November 2005
\bibitem{paper:query-language} ``XQuery 1.0: An XML Query Language'', W3C Candidate Recommendation, 3 November 2005
\bibitem{paper:query-fo} ``XQuery 1.0 and XPath 2.0 Functions and Operators'', W3C Candidate Recommendation, 3 November 2005
\bibitem{paper:query-serialization} ``XSLT 2.0 and XQuery 1.0 Serialization'', W3C Candidate Recommendation, 3 November 2005
