...
For example, \verb!CREATE DOCUMENT! "mail1" \verb!IN COLLECTION! \verb!"mails"! creates a document named "mail1" in the collection named "mails".
...
You can retrieve various metadata about database objects (such as documents, collections, indexes, etc.) by querying system documents and collections listed below. The documents marked with (*) are not persistent but generated on the fly. You can query these documents as usual but you cannot update them. Also these documents are not listed in \verb!$documents!.
...
\item A node-level before trigger that does not intend to cause either of these behaviors must be careful to return as its result the same node that was passed in (that is, the \verb!$NEW! node for \verb!INSERT! and \verb!UPDATE! triggers, for \verb!DELETE! triggers Ц its returned value is ignored in all cases except it is an empty sequence). 
\end{itemize}
