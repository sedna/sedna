...
% ѕозвол€ет вставл€ть картинки в формате eps.
\usepackage[dvips]{epsfig}

%используетс€ дл€ создани€ таблиц со слепленными клетками 
\usepackage{multirow}

% Ќиже следующие две строчки позвол€ют имен€ть пол€ документа.
%\addtolength{\hoffset}{31pt}
%\addtolength{\textwidth}{104pt}

\title{Sedna Programmer's Guide}
\date{}

\begin{document}
% Ёто однострочный комментарий. ћногострочных комментариев, к сожаление, в латехе нет.

% Ќиже следующий \sloppy ничего по сути не мен€ет, он только делает латех менее предирчивым к 
% оформлению документа. \sloppy лучше вставл€ть всегда.
\sloppy

% ¬ставка титульной страницы. ≈е содержимое определ€етс€ командами \title{},
%\author{}, \date{}, которые вставл€ютс€ до \begin{document}
\maketitle
...
%\part{(ѕример части) Ќазвание первой части}
%\chapter{(ѕример главы) «накомство с \LaTeX}
...
%The methods return true if the query was not an update, and returns false if that was an update. The methods throw \verb!DriverException! if the execution fails.
...
// begin transaction
        ...
        // execute a statement. the statement is a bluk load.
         ...
         boolean call_res = 
           st1.execute("LOAD \"region.xml\" \"region\"");
...
\begin{verbatim}
[win:] INSTALL_DIR\examples\applications\java\Client.java
\end{verbatim}
\begin{verbatim}
[linux:] INSTALL_DIR/examples/applications/java/Client.java
\end{verbatim}
...
the background mode".
\item Create a new database \verb!sample-db! by running the following command:
\begin{verbatim}
se_cdb sample-db
\end{verbatim}
If the database is created successfully it prints "The database 'sample-db'
has been created successfully".
\item Start the sample-db database by running the following command:
\begin{verbatim}
se_sm sample-db
\end{verbatim}
...
\item To compile the example, run the script
 ...
 \begin{verbatim}[linux:] Clientbuild.sh\end{verbatim}
...
\item  To run the compiled example, use the script
 ...
 \begin{verbatim}[linux:] Client.sh\end{verbatim} 
 located in the same folder as \verb!Client.java!.
...
libsedna library is supplied with header file: \verb!"libsedna.h"!. Client programs that use libsedna must include the header file libsedna.h, must link with the libsedna library and provide the compiler with the path to the directory where \verb!"libsedna.h"! files are stored.   

Sedna XML DBMS is supplied with libsednamt library also. libsednamt library is libsedna library compiled in multithreaded mode. Applications that need to be compiled in multithreaded mode must link with libsednamt library.

Below in the example we consider libsedna (single-threaded library).
...
\begin{note}
The initialization with \verb!SEDNA_CONNECTION_INITIALIZER! is not mandatory if you are using Sedna version 0.5 or earlier. 
\end{note}
...
\verb!SEDNA_ERROR! - some error is occured, session is not open.
...
\verb!SEDNA_ERROR! - some error occured.
...
\subsubsection{Transactions}
Application can execute queries and updates against the specified database only in the scope of a transaction. That is, once a session has been opened, application can begin a transaction, execute statements and commit this transaction. 
In a session transactions are processed sequentially. That is, application must commit a begun transaction before beginning a new one. To specify transaction boundaries application uses functions \verb!SEbegin!, \verb!SEcommit!:
...
To rollback transaction use \verb!SErollback! function:
...
\begin{verbatim}int SEexecuteLong(SednaConnection* conn, FILE* query_file)\end{verbatim}

parameters:

\verb!conn! - a pointer to an instance of the SednaConnection type.

\verb!query_file! - a pointer to the open file with the statement to execute.
...
int SEloadData(SednaConnection* conn, 
                     const char* buf, 
                   int bytes_to_load, 
                const char* doc_name)\end{verbatim}
...
\verb!SEDNA_ERROR! - some error occured. Data were not loaded.
...
If the function returns negative value, application can get the error info in the form of string using \verb!SEgetLastErrorMsg! function:
...
The function returns the information about the last error occured in this session.
 ...
 * C-API using libsedna library.
 * The application opens a session to "sample-db" database, 
 * begins a transaction, loads document
 * "region.xml" to "sample-db" database, executes XQuery query, 
 * retrieves every item of the 
 * result data, commits the  transaction and closes the session.
 */
...
The source code of this example program can be found in 
\begin{verbatim}
[win:] INSTALL_DIR\examples\applications\c\Client.c
\end{verbatim}
\begin{verbatim}
[linux:] INSTALL_DIR/examples/applications/c/Client.c
\end{verbatim}
...
the message about the error occured.
...
If any exception occured during query execution, the function sends rollback
...
[linux:] INSTALL_DIR/driver/scheme
\end{verbatim}
where \verb!INSTALL_DIR! refers to the directory where Sedna is installed.
...
\subsubsection{XQuery Support}
\label{sec:xquery-support}
The Sedna query language is XQuery \cite{paper:query-language} developed by W3C. 
Below is the list of the XQuery features, supported in the current release of Sedna.
\begin{itemize}
\item	Data model \cite{paper:data-model} 
\begin{itemize}
\item Nodes: Element, Document, Text, Namespace and Attribute Nodes.
\item Accesors: dm:node-kind, dm:node-name, dm:string-value, dm:typed-value
\item Atomic types:  xs:string , xs:boolean , xs:decimal (\emph{restricted}), xs:float, xs:double, xs:QName, xdt:untypedAtomic and xdt:anyAtomicType .
\end{itemize}
\item	Language features \cite{paper:query-language} 
\begin{itemize}
\item Primary Expressions: literals, variable references, parenthesized expressions, context item expression, XQuery comments
\item Path Expressions
\begin{itemize}
\item Axes: child, descendant, attribute, self, descendant-or-self, parent.
\item Node Tests: NameTest, Wildcard , TextTest, AnyKindTest.
\item Other Features: predicates, abbriviated syntax.
\end{itemize}
\item Sequences: comma operator
\item Arithmetic Expressions
\item Comparisons: value comparison, general comparison, node comparison.
\item Logical Expressions
\item Constructors: direct element constructor, computed element and attribute constructors.
\item FLWOR: for and let clause, where clause, return clause.
\item Function declaration (locally defined user functions are supported).
\item Other Expressions: conditional expression, quantified expression (without type declaration), cast expression.
\end{itemize}
\item Functions and operators \cite{paper:query-fo}: see \ref{xquery-functions}
\end{itemize}
...
\begin{verbatim}
UPDATE
insert Expr1 (INTO|PRECEEDING|FOLLOWING) Expr2
\end{verbatim}
...
\begin{verbatim}
UPDATE
rename //job[.="bit banger"] on profession
\end{verbatim}
...
\begin{verbatim}
LOAD "file_name" "document_name" 
\end{verbatim}

The first parameter is a file name, it can be an absolute path to the file or only file name if the file is in the local directory. The second parameter is the name for this document in the database.

\begin{verbatim}
LOAD "file_name" "document_name" "collection_name"
\end{verbatim}

The first parameter is a file name, it can be an absolute path to the file or only file name if the file is in the local directory. The second parameter is the name for this document in the database. The third parameter is the collection name to load the document into.
...
By default, Sedna removes boundary whitespaces according to the boundary-space policy defined in \cite{paper:query-language}. To control boundary whitespaces processing, use \emph{boundary-space declaration} in the prolog of the \verb!LOAD! statement as specified in \cite{paper:query-language}.  The following example illustrates a boundary-space declaration that instructs Sedna to preserve whitespaces:
\begin{verbatim}
...
\begin{itemize}
\item avg, concat, count, contains, data, distinct-values, document-uri, sum, max, min, true, false, not, empty, exists, string, string-length, translate
...
%==========================================================================
\subsubsection{Index-scan Functions}
\label{sec:index-scan-fun}
In the current version of Sedna, query executor does not use indecies automatically. Use the following functions
...
Sometimes it is more convenient or more efficient to use prepared SQL statemets
...
libraries (i.e. \verb!.dll! files in Windows or \verb!.so! files in Linux) 
...
\begin{itemize}
	...
	It may also be used to allocate memory for iternal use, such memory must be
...
\begin{verbatim}
[win:] INSTALL_DIR\examples\applications\external-functions\c\
\end{verbatim}
\begin{verbatim}
[linux:] INSTALL_DIR/examples/applications/external-functions/c/
\end{verbatim}
where \verb!INSTALL_DIR! refers to the directory where Sedna is installed.
...
This section describes Sedna Data Definition Language (DDL) that is used to create the database structures that will hold oneТs data.
...
\subsubsection{Managing Documents}
\begin{verbatim}
...
Collections are preferrable for situations when a group of documents is to be
...
%To drop a document located in collection, the following statement is provided:
...
\subsubsection{Managing Indices}
\label{sec:managing-indices}
Sedna supports value indicies based on B-tree to index XML element content and attribute values. Below is the description of statements to manage indicies.
...
\begin{note}In the current version of Sedna, query executor does not use indicies automatically. You can enforce the executor to employ indices by
using the XQuery index-scan funtions specified in Section \ref{sec:index-scan-fun}.
\end{note}

\subsubsection{Retrieving Metadata}
\begin{verbatim}
RETRIEVE METADATA FOR DOCUMENTS [WITH STATISTICS]
RETRIEVE METADATA FOR COLLECTIONS [WITH STATISTICS]
\end{verbatim}

The \verb!RETRIEVE! \verb!METADATA! statement retrieves the metadata information about all XML documents or collections stored in database. The output of this statement is an XML document, which complies with the following DTD:

\begin{verbatim}
<!ELEMENT XML_DOCUMENTS (Document)*|(Collection)*>
...
The value of the \verb!name! attribute is the name of the document or collection. If the \verb!WITH! \verb!STATISTICS! command is stated, the following statictical information is retrieved as the content of the \verb!Document! or \verb!Collection! elements:

\begin{itemize}
\item \verb!TOTAL NODES IN SCHEMA!. The number of the nodes of the descriptive schema.
\item \verb!TOTAL TEXT NODES IN SCHEMA!. The number of the text attribute and document nodes of the descriptive schema.
\item \verb!TOTAL NODES!. The number of the nodes of the document (or collection).
\item \verb!DEPTH!. The maximal depth of the document (or collection).
\item \verb!TOTAL BLOCKS EXCEPT NID! and \verb!INDIR!. The number of the blocks occupied by document (or collection) except blocks with numberig schema and indirection.
\item \verb!TOTAL DESCRIPTOR BLOCKS!. The number of  the descriptor blocks occupied by document (or collection) .
\item \verb!TOTAL STRING BLOCKS!. The number of  the text blocks occupied by document (or collection) .
\item \verb!FILL PERCENTAGE!. Fill factor of the blocks (in percents).
\item \verb!TOTAL INNER BLOCKS!. The number of  the descriptor blocks occupied by document (or collection) except first and last blocks in each chain of blocks.
\item \verb!TOTAL INNER NODES SIZE!. The size of the inner blocks.
\item \verb!INNER FILL PERCENTAGE!. Fill factor of the inner blocks (in percents).
...
\item \verb!NIDS percentage!. The share of  the long labeling numbers' (>11) total size (in percents).
\item \verb!INDIRECTION percentage!. The share of  the indirection records' total size (in percents).
\item \verb!TOTAL SIZE!. The total size of the document (or collection), in MGBs.
\item \verb!STRINGS size!. The total size of the string blocks , in MGBs.
\item \verb!DESCRIPTORS size!. The total size of the descriptor blocks , in MGBs.
\item \verb!NIDS size!. The total size of the long labeling numbers , in MGBs.
\item \verb!FREE SPACE IN STRING BLOCKS size!. The total size of the free space in the string blocks.
\item \verb!INDIRECTION size!. The total size of the indirection records , in MGBs.
\item \verb!NIDS size!. The share of  the indirection records' total size (in percents).
\item \verb!Data size variation!. The histogram of the xml data by it's size.
\item \verb!NID size variation!. The histogram of the labeling numbers it's length.
\end{itemize}

\begin{verbatim}
RETRIEVE DESCRIPTIVE SCHEMA FOR DOCUMENT doc-name-expr
RETRIEVE DESCRIPTIVE SCHEMA FOR COLLECTION coll-name-expr
\end{verbatim}

The \verb!RETRIEVE! \verb!DESCRIPTIVE! \verb!SCHEMA! statement retrieves the descriptive schema of the XML document or collection with the \verb!doc-name-expr! or \verb!coll-name-expr! name respectively. 

%The output format of the descriptive schema is specified after the \verb!AS! clause. In the current version of Sedna there is only the TREE format. In future versions more formats will be added. 

The output of a descriptive schema is an XML document, which complies with the following DTD:

\begin{verbatim}
<!ELEMENT TREE_DESCRIPTIVE_SCHEMA NODE>
<!ELEMENT NODE (NODE)*>
<!ATTLIST NODE name CDATA #IMPLIED
               type (document|element|text|attribute)>
\end{verbatim}

The root of the XML document is the \verb!TREE_DESCRIPTIVE_SCHEMA! element, which contains the tree representation of a descriptive schema. The \verb!NODE! element describes a node of descriptive schema. It has two attributes: \verb!name! and \verb!type!, which define the name and the type of the node correspondingly. There are four types of nodes in descriptive schema: document, element, attribute and text. More on descriptive schema see in \cite{paper:sedna-overview}.

\begin{thebibliography}{9}
\bibitem{doc:admin} ``Sedna Administration Guide''
\bibitem{paper:data-model} ``XQuery 1.0 and XPath 2.0 Data Model'', W3C Working Draft, 4 April 2005
\bibitem{paper:query-language} ``XQuery 1.0: An XML Query Language'', W3C Working Draft, 4 April 2005
\bibitem{paper:query-fo} ``XQuery 1.0 and XPath 2.0 Functions and Operators'', W3C Working Draft, 4 April 2005
\bibitem{paper:query-serialization} ``XSLT 2.0 and XQuery 1.0 Serialization'', W3C Working Draft, 15 September 2005
