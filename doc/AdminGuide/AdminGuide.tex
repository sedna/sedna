...
options:
  ...
  -echo on/off          display se_term output (default on)
  -show-time on/off     show time of query execution (default off)
...
Typing \verb!-help! or \verb!--help! Administrator provides the exhaustive information about the \verb!se_term!.
...
The \verb!-echo! option specifies if \verb!se_term! output needs to be displayed or not. If the \verb!echo! is on, the output is displayed, if the \verb!echo! is off, the output is not displayed. By default, the \verb!echo! option is set to on.

The \verb!-show-time! option allows to get the time of the latest query execution. If used with the \verb!-query! option provides the time of the specified query execution; if used with the \verb!-file! option provides the time of the execution of the last query in file.
...
\verb!se_term! returns 0 to the shell if it finished normally, 1 if there were errors.

When \verb!se_term! is used in the interactive mode it takes in query/update statements and meta-commands. To execute a query/update statement type in a statement and use semicolon and line feed to terminate it (\verb!;'\n'! terminates the query). An end of line does not terminate the query, thus queries can be spread over several lines for clarity. The first query/update statement automatically begins transaction. If the statement executed successfully, the results are displayed on the screen.

Meta-commands are commands for \verb!se_term! that proccessed by the \verb!se_term! itself. A meta-command begins with a backslash and that differs it from the query/update statements. Thus, the format of \verb!se_term! meta-command is the backslash, followed immediately by a command, with no semicolon at the end. \verb!se_term! takes in the following meta-commands:
