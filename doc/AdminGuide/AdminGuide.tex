...
options:
  ...
  -background-mode on/off    start the server in the background mode
                             (default on)
  -address-to-listen iface   socket listening address or interface name
                             (default localhost)
...
\verb!-address-to-listen! option specifies an interface which Sedna listens for
connections from clients. You may use numeric form (e.g. \verb!127.0.0.1!) or
human-readable form (e.g. \verb!localhost!) that could be resolved using
/etc/hosts as listening address. Note that address must be local. If address
is not specified Governor tries to find the \verb!sednaconf.xml! file and uses
the address specified as \verb!listener_port! parameter (see section
...
<!ELEMENT listener_address (#PCDATA)>
...
The \verb!listener_address! element defines the address Sedna tries to listen
