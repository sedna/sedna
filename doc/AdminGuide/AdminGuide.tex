...
\item The \verb!<db_name>.seph! and \verb!<db_name>.ph.sebu! files are used for storing XML metadata (for example, the descriptive scheme of XML documents).
...
\verb!-debug! option specifies session debug mode. If \verb!-debug! option is on, statements of this session are exectued in a debug mode. If \verb!-debug! option is off, statement of this session are executed in normal (not debug) mode. For details see "Debug Facilities" section of the Sedna Programmer's Guide.
