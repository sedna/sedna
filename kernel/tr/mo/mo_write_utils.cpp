...
void deleteBlock(node_blk_hdr * block)
{
    ...
    {
      CHECKP(bladdr);
      schema_node_cptr scm(GETSCHEMENODEX(bladdr), true);
      UPDATEFIRSTBLOCKPOINTER(scm,tmp1);
    }
    block->snode.modify()->blockcnt--;
...
xptr createNewBlock(schema_node_cptr scm)
{
    ...
    switch (type)
    {
        case element:  dscsz=scm->children.count()*sizeof(xptr)+sizeof(e_dsc);
            break;
        case attribute: dscsz=sizeof(a_dsc);
            break;
        case text   :case comment: case cdata: dscsz=sizeof(t_dsc);
            break;
        case document: case virtual_root: dscsz=scm->children.count()*sizeof(xptr)+sizeof(d_dsc);
            break;
...
xptr createBlockNextToTheCurrentWithAdvancedDescriptor(node_blk_hdr* block)
{
    ...
    node_blk_hdr* tmp= se_new node_blk_hdr;
    *tmp=*block;
    tmp->dsc_size=tmp->snode->children.count()*sizeof(xptr)+size_of_node(tmp);
    xptr new_block;
    bool persistent= IS_DATA_BLOCK(old_blk);
    if (persistent)
    {
...
xptr createBlockPriorToTheCurrentWithAdvancedDescriptor(node_blk_hdr* block)
{
    ...
    *tmp=*block;
    ...
    CHECKP(new_block);
    new_block_hdr->snode.modify()->blockcnt++;
    //if (persistent)       add_new_block(new_block);
...
void addChildsBySchemeSplittingBlock(xptr parent, const char* name,t_item type, xptr child, xmlns_ptr ns)
{
        ...
        /*&& CALCSHIFT (XADDR(parent),parent_block)==sizeof(node_blk_hdr)*/
    {
        ...
        *((shft *)ptr)=0;
        ...
        parent_block->dsc_size=size_of_node(parent_block)+sizeof(xptr)*parent_block->snode->children.count();
...
/*
    ...
    */
        xptr * child_ptr = elementContainsChild((n_dsc*)(((char*)parent_block)+sizeof(node_blk_hdr)),name,type,ns);
        U_ASSERT(child_ptr != NULL);
        *child_ptr = child;
    }
    else
    {
        ...
        if (getNearestBorder(parent_block,(n_dsc*)XADDR(parent))>0)
        {
            ...
            if (nblk!=XNULL)
            {
                ...
                if (nblk_hdr->dsc_size>=size && nblk_hdr->free_first!=0)
                {
                    ...
                    int posit=parent_block->snode->find_first_child(ns,name,type);
...
void insertTextValue(xptr node,const void* text, unsigned int size,text_type ttype)
{
    ...
    else
    {
        ...
        if ((cur_size+size)>PSTRMAXSIZE)
        {
            ...
            char* z=se_new char[cur_size];
            memcpy(z,data,cur_size);
            t_dsc old_node_buf, new_node_buf;
            CHECKP(node);
            memcpy(&old_node_buf, XADDR(node), sizeof(t_dsc));
            pstr_long_create_str(node, text,  size,ttype);
            pstr_long_append_tail(node, z, cur_size,text_mem);
            //FIXME: change pstr and remove redundant copying
            CHECKP(node);
            memcpy(&new_node_buf, XADDR(node), sizeof(t_dsc));
            memcpy(XADDR(node), &old_node_buf, sizeof(t_dsc));
            pstr_deallocate(node);
            CHECKP(node);
            VMM_SIGNAL_MODIFICATION(node);
            memcpy(XADDR(node), &new_node_buf, sizeof(t_dsc));
...
void update_idx_add (xptr node)
{
    ...
    cat_list<index_ref>::item* ind=scm->index_list.first;
    while (ind != NULL)
    {
        ind->object.index->put_to_index(node,ind->object.object);
        RECOVERY_CRASH;
        ind=ind->next;
    }
    CHECKP(node);
...
void update_idx_add (xptr node,const char* value, int size)
{
    ...
    while (ind!=NULL)
    {
        ind->object.index->put_to_index(node,value,size,ind->object.object);
        RECOVERY_CRASH;
        ind=ind->next;
    }
    CHECKP(node);
...
void update_idx_add_txt (xptr node)
{
    ...
    schema_node_cptr scm=(GETBLOCKBYNODE(node))->snode;
    cat_list<index_ref>::item * ind=scm->index_list.first;
    if (ind==NULL&&scm->parent->index_list.empty()) return;
    while (ind!=NULL)
    {
        ind->object.index->put_to_index(node,ind->object.object);
        RECOVERY_CRASH;
    ...
    ind=scm->index_list.first;
    if (ind==NULL) return;
    xptr pnode=removeIndirection(((n_dsc*)XADDR(node))->pdsc);
    while (ind!=NULL)
    {
        ind->object.index->put_to_index(pnode,ind->object.object);
        RECOVERY_CRASH;
        ind=ind->next;
    }
    CHECKP(node);
...
void update_idx_add_txt (xptr node,const char* value, int size)
{
    ...
    while (ind!=NULL)
    {
        ind->object.index->put_to_index(pnode,value,size,ind->object.object);
        RECOVERY_CRASH;
        ind=ind->next;
    }
    CHECKP(node);
...
void update_idx_delete (xptr node)
{
    ...
    while (ind!=NULL)
    {
        ind->object.index->delete_from_index(node,ind->object.object);
        RECOVERY_CRASH;
        ind=ind->next;
    }
}
...
void update_idx_delete_text (schema_node_cptr scm,xptr node,const char* value, int size)
{
    ...
    while (ind!=NULL)
    {
        ind->object.index->delete_from_index(node,z,size,ind->object.object);
        ind=ind->next;
    }
    // parent
    if (scm->type!=text)
    ...
    while (ind!=NULL)
    {
        ind->object.index->delete_from_index(pnode,z,size,ind->object.object);
        ind=ind->next;
    }
    delete [] z;
}
    ...
    if (pr_blk->indir_count>=pr_blk->count&& 
            ...
            pr_blk->snode->bblk_indir==pr_blk->sm_vmm.p))
        {
            ...
            if (pr_blk->snode->bblk_indir==pr_blk->sm_vmm.p)
                pr_blk->snode.modify()->bblk_indir=r_bl;
            else
            if (l_bl!=XNULL)
            {
                CHECKP(l_bl);
...
void increment_count(node_blk_hdr* pr_blk,shft count)
{
    ...
    if (pr_blk->indir_count<pr_blk->count&& 
            ...
            pr_blk->snode->bblk_indir!=pr_blk->sm_vmm.p))
        {
            ...
            pr_blk->snode.modify()->bblk_indir=pr_blk->sm_vmm.p;
            xptr r_bl=pr_blk->nblk_indir;
            if (r_bl!=XNULL)
            {
