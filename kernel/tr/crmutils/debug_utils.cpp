...
void printMFO (schema_node* node, std::map<schema_node*, std::pair<int,int> > &mfo,int par_pref,int indent)
{
	...
	while (sc!=NULL)
	{
		printMFO(sc->snode,mfo,size,indent+1);
		sc=sc->next;
	}		
}
void getNIDNEWDistribution(std::map<schema_node*,int> &xsfo,
						   std::map<schema_node*,int> &ncnt,
						   std::map<int,int> &nidsz,
						   schema_node* node, int par_pref)
{
	int size=1;
	if (node->parent!=NULL)
		size=par_pref+
	...
	if (it!=nidsz.end())
		...
		nidsz[size]=ncnt[node];
	//recursive walkthrough
	sc_ref*	sc= node->first_child;
	while (sc!=NULL)
	{
		getNIDNEWDistribution(xsfo,ncnt,nidsz,sc->snode,size);
		sc=sc->next;
	}		
}
void printNIDVariation(xptr root, se_ostream& crmout)
{
	...
	while(node!=XNULL)
	{
		...
		CHECKP(node);
		//scheme cnt
		schema_node* sc =(GETBLOCKBYNODE(node))->snode;
		std::map<schema_node*,int>::iterator cit=ncnt.find(sc);
		if (cit!=ncnt.end())
		{
			ncnt[sc]++;
		}
		...
		while (tmp==XNULL /*&& GETPARENTPOINTER(node)!=XNULL*/)
		{
			...
			std::map<schema_node*,int>::iterator it=xsfo.find(scm);
			if (it!=xsfo.end())
			{
	...
	nidsz.clear();
	CHECKP(root);
	schema_node* sc =(GETBLOCKBYNODE(root))->snode;
...
void printFullNIDVariation(xptr broot, xptr node)
{
	...
	xptr parent=insert_element(node,XNULL,XNULL,"NID",xs_untyped,NULL);
	char buf[20];
	u_itoa(maxfo,buf,10);
	xptr left=insert_attribute(XNULL,XNULL,parent,"max_fan_out",xs_untypedAtomic,buf,strlen(buf),NULL);
	u_gcvt((1.*midfo/cnt),10,buf);
	left=insert_attribute(left,XNULL,XNULL,"mid_fan_out",xs_untypedAtomic,buf,strlen(buf),NULL);
	...
	left=insert_attribute(left,XNULL,XNULL,"total_size",xs_untypedAtomic,buf,strlen(buf),NULL);
	u_gcvt((cnt_out/1024./1024.),10,buf);
	left=insert_attribute(left,XNULL,XNULL,"total_size_ext",xs_untypedAtomic,buf,strlen(buf),NULL);
	
	
	left=insert_element(left,XNULL,XNULL,"histogram",xs_untyped,NULL);
	xptr lf=XNULL;
    int i=1;
	for (i=1;i<=max;i++)
	{
		...
		if (it!=nidsz.end())
		{
			...
			att=insert_attribute(att,XNULL,XNULL,"total",xs_untypedAtomic,buf,strlen(buf),NULL);
			u_gcvt((100.*it->second/cnt),10,buf);
			insert_attribute(att,XNULL,XNULL,"percentage",xs_untypedAtomic,buf,strlen(buf),NULL);
}
		else
		{
			lf=insert_element(lf,XNULL,left,"bucket",xs_untyped,NULL);
	...
	parent=insert_element(parent,XNULL,XNULL,"STRINGS",xs_untyped,NULL);
	u_gcvt((cnt_sz/1024./1024.),10,buf);
	left=insert_attribute(XNULL,XNULL,parent,"total_size",xs_untypedAtomic,buf,strlen(buf),NULL);
	u_itoa(strcnt,buf,10);
	left=insert_attribute(left,XNULL,XNULL,"total_count",xs_untypedAtomic,buf,strlen(buf),NULL);
	left=insert_element(left,XNULL,XNULL,"histogram",xs_untyped,NULL);
	lf=XNULL;
	for (i=0;i<=max;i++)
	{
...
{
	xptr left=insert_element(neighbour,XNULL,XNULL,title,xs_untyped,NULL);
#ifdef WIN32
...
void getDebugInfo(schema_node* snode, xptr& node)
{
	...
	xptr left=insert_element(XNULL,XNULL,node,"total_schema_nodes",xs_untyped,NULL);
...
void getDebugInfo(schema_node* snode, debug_info* d_in)
{
	...
	sc_ref*	sc= snode->first_child;
	while (sc!=NULL)
	{
		getDebugInfo(sc->snode, d_in);
		sc=sc->next;
	}
...
void getSimpleDebugInfo(schema_node* snode, debug_info* d_in)
{
	...
	while (sc!=NULL)
	{
		getSimpleDebugInfo(sc->snode, d_in);
		sc=sc->next;
	}
	if (d_in->cdp>d_in->mdpth) d_in->mdpth=d_in->cdp;
	d_in->cdp--;
...
void isSchemaPCAllRight(schema_node* snode)
{
	...
	while (sc!=NULL)
	{
		if (sc->snode->parent!=snode)
		{
			crm_out<<"Error";
		}
		isSchemaPCAllRight(sc->snode);
		sc=sc->next;
	}
}
...
void checkBlockSequenceConsistency(schema_node* snode, se_ostream& crmout)
{
	...
	while (sc!=NULL)
	{
		checkBlockSequenceConsistency(sc->snode, crmout);
		sc=sc->next;
	}
}
...
{
	schema_node* rscm=find_document(docname);
	if (rscm!=NULL)
	{
