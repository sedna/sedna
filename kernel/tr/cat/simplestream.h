...
protected:
    ...
    void grow (se_size_t garanteed_size = 128) {
...
public:
    ...
    inline void write(const void * s, se_size_t length) {
 ...
 *  It can occupy 1, 2 or sizeof(se_size_t) bytes.
 *  If length is less then 255, it is stored in 1 byte
 *  otherwise, it is stored in sizeof(se_size_t) bytes 
 ...
 */
    void read_string(se_size_t len, char * c) {
    ...
    bool read(void * s, se_size_t length) {
    ...
    inline void seekto(char stopat = '\0') { 
        ...
        se_size_t len = m_length - m_position, i = 0;
    ...
    inline void set_capacity(se_size_t new_capacity) { grow(new_capacity); };
