...
{
    PPFnMaxMin *res = new PPFnMaxMin(_cxt_, info, i, child, collation);
...
{
    PPFnSumAvg *res = new PPFnSumAvg(_cxt_, info, i, child, zero);
    res->child.op = child.op->copy(_cxt_);
    if (zero.op)
...
{
    PPFnCount *res = new PPFnCount(_cxt_, info, child);
    res->child.op = child.op->copy(_cxt_);
    return res;
}
