...
(define (sc:auth-query-rewriting expr user is-authentication is-authorization)
  
  ; if e  - empty list then return empty list
   (if (eq? expr '())
         '()

         ; if database was created with -db-security=off then no rewritings, check if it a security query, then throw exception
         (if (eq? is-authentication 0)
             (case (caaddr expr)
               ((create-user 
                 drop-user 
                 alter-user)
           ...
           ( (eq? (car expr) 'manage)   
             (begin
               ...
               (cond
                 ...
                 ( (eq? (caaddr expr) 'create-collection) 
                   (if (eq? is-authorization 0)
                       (list expr)                    
                 ...
                 ( (eq? (caaddr expr) 'drop-collection) 
                   (if (eq? is-authorization 0)
                       (list expr) 
