...
if test -d "bin"; then
  ...
  printf "se_rc, se_sm, se_smsd, se_stop, se_term, se_trn, se_rcv, se_exp, se_hb? "
  read yesno
  case "$yesno" in
    [yY]* ) sysdir="$where" ;;
    * ) sysdir="" ;;
  ...
  echo "se_sm, se_smsd, se_serv, se_term, se_trn, se_rcv, se_exp, se_hb then enter the prefix you "
  echo "want to use."
...
if test ! "x$sysdir" = "x"; then
  ...
  if test -d "bin" && test -w "bin"; then
    ...
    for x in "se_cdb" "se_ddb" "se_gov" "se_rc" "se_sm" "se_smsd" \
             "se_stop" "se_term" "se_trn" "se_rcv" "se_exp" "se_hb"; do
